/datum/objective/personal/grave_robbery
	name = "Rob Graves"
	category = "Mask's Chosen"
	triumph_count = 2
	rewards = list("2 Triumphs", "Mask grows stronger", "Ability to rob graves without being cursed", "Mask blesses you (+1 Endurance)")
	var/graves_robbed = 0
...
/datum/objective/personal/grave_robbery/proc/on_grave_robbed(datum/source, mob/user)
	...
	if(graves_robbed >= graves_required)
		...
		to_chat(owner.current, span_notice("Grave robbed! Rob [graves_required - graves_robbed] more to complete Mask's task."))

/datum/objective/personal/grave_robbery/complete_objective()
	. = ..()
	to_chat(owner.current, span_greentext("You've robbed enough graves to earn Mask's respect!"))
...
/datum/objective/personal/grave_robbery/update_explanation_text()
	explanation_text = "Rob at least [graves_required] graves to earn Mask's respect."
