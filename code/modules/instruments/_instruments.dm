...
/obj/item/instrument/attack_self(mob/living/user, list/modifiers)
	...
	if(HAS_TRAIT(user, TRAIT_BARDIC_TRAINING)) // Non-bards will never get this prompt. Prompt doesn't show if you cancel song selection either.
		...
		switch(music_level) // There has to be a better way to do this, but so far all I've tried doesn't work as intended.
			if(1) // T1
				buffs2pick += list("Mystra's Brilliance (+1 INT)" = /datum/status_effect/bardicbuff/intelligence)
			if(1 to 2) // T2
				buffs2pick += list("Mystra's Brilliance (+1 INT)" = /datum/status_effect/bardicbuff/intelligence,
								"Moradin's Resilience (+1 END)" = /datum/status_effect/bardicbuff/endurance)
			if(1 to 3) // T3
				buffs2pick += list("Mystra's Brilliance (+1 INT)" = /datum/status_effect/bardicbuff/intelligence,
								"Moradin's Resilience (+1 END)" = /datum/status_effect/bardicbuff/endurance,
								"Ilmater's Blessing (+1 CON)" = /datum/status_effect/bardicbuff/constitution)
			if(1 to 4) // T4
				buffs2pick += list("Mystra's Brilliance (+1 INT)" = /datum/status_effect/bardicbuff/intelligence,
								"Moradin's Perseverance (+1 END)" = /datum/status_effect/bardicbuff/endurance,
								"Ilmater's Blessing (+1 CON)" = /datum/status_effect/bardicbuff/constitution,
								"Tymora's Alacrity (+1 SPD)" = /datum/status_effect/bardicbuff/speed)
			if(1 to 5) // T5
				buffs2pick += list("Mystra's Brilliance (+1 INT)" = /datum/status_effect/bardicbuff/intelligence,
								"Moradin's Perseverance (+1 END)" = /datum/status_effect/bardicbuff/endurance,
								"Ilmater's Blessing (+1 CON)" = /datum/status_effect/bardicbuff/constitution,
								"Tymora's Alacrity (+1 SPD)" = /datum/status_effect/bardicbuff/speed,
								"Tempus's Righteous Fury (+1 STR, +1 PER)" = /datum/status_effect/bardicbuff/ravox)
			if(6 to INFINITY) // Legendary onwards
				buffs2pick += list("Mystra's Brilliance (+1 INT)" = /datum/status_effect/bardicbuff/intelligence,
								...
								"Tymora's Alacrity (+1 SPD)" = /datum/status_effect/bardicbuff/speed,
								"Tempus's Righteous Fury (+1 STR, +1 PER)" = /datum/status_effect/bardicbuff/ravox,
								"Lathander's Awakening (+energy, +stamina, +1 FOR)" = /datum/status_effect/bardicbuff/awaken) // TAKE THE LAND THAT MUST BE TAKEN
			else // debug
...
/obj/item/instrument/lute
	name = "lute"
	desc = "The favored instrument of Sune, made of wood and simple string."
	possible_item_intents = list(MACE_WDSTRIKE)
	force = 5
	icon_state = "lute"
	item_state = "lute"
	song_list = list(
	"Umberlee's Second Shanty" = 'sound/instruments/band/lute (b1).ogg',
...
/obj/item/instrument/accord
	name = "accordion"
	desc = "A complex piece of dwarven intuition, composed of metal, wood, hide and ivory. Favored by Umberlee's bards."
...
/obj/item/instrument/flute // small rats approach a little when begin playing
	...
	song_list = list(
	"Umberlee's Second Shanty" = 'sound/instruments/band/flute (b1).ogg',
...
/obj/item/instrument/viola
	...
	icon_state = "viola"
	song_list = list(
	"Umberlee's Second Shanty" = 'sound/instruments/band/viola (b1).ogg',
...
/obj/item/instrument/vocals
	...
	"Jergal's Lullaby (Feminine)" = 'sound/instruments/vocalsf (2).ogg',
