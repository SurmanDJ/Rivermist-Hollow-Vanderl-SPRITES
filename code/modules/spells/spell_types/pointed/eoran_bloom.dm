/datum/action/cooldown/spell/eoran_bloom
	name = "Sharess' Bloom"
	desc = "Grows an Sharess' bud on the target."
	...
	invocation = "Be at peace with Sharess."
...
/datum/action/cooldown/spell/eoran_bloom/cast(atom/cast_on)
	...
	if(!to_pacify)
		...
		if(!spawn_on.density)
			...
			to_chat(owner, span_warning("The targeted location is blocked. The flowers of Sharess refuse to bloom."))
		return

	if(to_pacify.get_item_by_slot(ITEM_SLOT_HEAD))
		to_chat(owner, span_warning("The target's head is covered. The flowers of Sharess need an open space to bloom."))
		return
	...
	to_chat(to_pacify, span_info("<b style='color:pink'>A flower of Sharess blooms on my head, I feel at peace.</b>"))

/obj/item/clothing/head/peaceflower
	name = "Sune bud"
	desc = "A flower of gentle petals, associated with Sune or Jergal. Usually adorned as a headdress or laid at graves as a symbol of love or peace."
...
/obj/item/clothing/head/corruptflower
	name = "blissaran bud"
	desc = "A flower of dark petals and sharp thorns, associated with Blissara. It is said that these allow their wearer to better commune with their goddess."
	icon = 'icons/roguetown/items/produce.dmi'
	mob_overlay_icon = 'icons/roguetown/clothing/onmob/head_items.dmi'
