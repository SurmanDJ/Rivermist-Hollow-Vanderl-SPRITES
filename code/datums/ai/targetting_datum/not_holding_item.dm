 ...
 *
 */
/datum/targetting_datum/basic/allow_items/not_holding_item
...
/datum/targetting_datum/basic/allow_items/not_holding_item/can_attack(mob/living/living_mob, atom/target, vision_range)
	...
	if(can_attack && !targetmob?.is_holding_item_of_type(object_type_path))
		return TRUE // they don't have the item
	// No valid target
	return FALSE
 ...
 *
 */
/datum/targetting_datum/basic/allow_items/holding_item
	/// BB key that holds the target typepath to use
	var/target_item_key = BB_TARGET_HELD_ITEM

///Returns true or false depending on if the target can be attacked by the mob
/datum/targetting_datum/basic/allow_items/holding_item/can_attack(mob/living/living_mob, atom/target, vision_range)
	var/datum/ai_controller/controller = living_mob.ai_controller
	var/object_type_path = controller.blackboard[target_item_key]
